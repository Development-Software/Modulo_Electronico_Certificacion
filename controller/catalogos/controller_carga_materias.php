...
if (isset($_FILES["file_materias"])) {
    $up = new Upload($_FILES["file_materias"]);
    if ($up->uploaded) {
        $up->Process("/tmp/");
        if ($up->processed) {
            ...
            for ($row = 2; $row <= $totReal; $row++) { //limite $highestRow se cambia por $totReal
                ...
                $x_clave_legal = trim($sheet->getCell("B" . $row)->getValue());
                $x_descripcion = trim($sheet->getCell("C" . $row)->getValue());
                $x_no_institucion = trim($sheet->getCell("D" . $row)->getValue());
                $x_active = trim($sheet->getCell("E" . $row)->getValue());


                $catalogo->setIdInstitucion($x_no_institucion);
                if (!empty(trim($x_no)) && !empty(trim($x_clave_legal)) && !empty(trim($x_descripcion)) && !empty(trim($x_no_institucion))) {
                    if (!(preg_match('/^\d+$/', $x_no))) {
                        $res = array("acc" => false, "msg" => "El id de la materia tiene un formato incorrecto.");
                        ...
                        if ($catalogo->existeCatalogoId("Id_Materia", "Materias", $x_no)) {
                            //Update registro
                            $update = $catalogo->udpCatalogoMaterias($x_no,$x_clave_legal, $x_descripcion, $x_no_institucion, $x_active);
                            if ($update) {
                            ...
                            //Insert registro
                            $insert = $catalogo->addCatalogoMaterias($x_no,$x_clave_legal, $x_descripcion, $x_no_institucion);
                            if ($insert) {
                                array_push($registros, $x_no);
                            } else {
                                array_push($registrosError, $x_no);
                        ...
                        //$registros[] = ['id' => $x_no, 'clave_legal' => $x_clave_legal, 'descripcion' => $x_descripcion, 'idInstitucion' => $x_no_institucion];
                    }
                } else {
